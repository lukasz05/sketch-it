...
import { createRoomCard } from "../../common/room-card.mjs";
...
function displayRooms(rooms) {
    ...
    if (rooms.length > 0) {
        ...
        const message = document.createTextNode("Nno rooms to display!");
    ...
    sendRoomCreation.addEventListener("click", () => {
        ...
        socket.emit(eventNames.CREATE_ROOM_REQUEST, roomName, {}, (response) => {
            if (response.success) {
                ...
                url.searchParams.append("id", roomName);
...
function validateUsername(username, errorNotification) {
    ...
    if (err) {
        errorNotification.innerText =
            "Playerr name must be between 3 and 10 alphanumeric characters.";
